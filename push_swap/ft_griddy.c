...
void mc_griddy(t_stacks *s)
{
	...
	while(!qie(s->b))
	{
		zd = (t_coord){s->a.head, s->b.head, 0, '\0', 0, '\0', 0, INT_MAX};
		xy = zd;
		while (!zd.bk)
		{
			tinder(s->a, &zd.bk);
			scrooge_mcrot(s, &zd);// non devi chiamarlo tu ma notsizethatcounts
			if(xy.totc > zd.totc)
				xy = zd;
			zd.bk = zd.bk->next;
		}
		blind_date(s, &xy);//will push the node once reached tops
	}
	final_sortami(s);
}
...
void scrooge_mcrot(t_stacks *s, t_coord *xy)
{
	...
	// zd = (t_coord){0};//for the funcs needed pass its add
